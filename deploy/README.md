...
Then create `/opt/scrobble-scrubber/.env` from `.env.example`, `chmod 600`, and **leave
`DRY_RUN=true`**.
...
ssh ubuntu@<host> 'cd /opt/scrobble-scrubber && node -e "
  const D=require(\"better-sqlite3\");const d=new D(\".data/scrobble-scrubber.sqlite\");
...
rsync -az --delete dist drizzle deploy package.json package-lock.json \
  ubuntu@<host>:/opt/scrobble-scrubber/
ssh ubuntu@<host> 'cd /opt/scrobble-scrubber && npm ci --omit=dev && sudo systemctl restart scrobble-scrubber'
ssh ubuntu@<host> 'sleep 5 && systemctl is-active scrobble-scrubber'
```
...
```sh
journalctl -u scrobble-scrubber -f
```
...
1. Restore the newest `predeploy_*.sqlite` over `.data/scrobble-scrubber.sqlite`, and delete the
...
- **Rollback does not undo Last.fm edits.** Those are irreversible no matter what this repo does;
  ...
  `DRY_RUN=true` and restart, or `sudo systemctl stop scrobble-scrubber`.

## Ops notes

- Logs: `journalctl -u scrobble-scrubber -f`. No log library; journald supplies timestamps.
